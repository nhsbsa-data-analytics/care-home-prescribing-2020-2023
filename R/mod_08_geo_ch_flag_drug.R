          ...
          nhs_grid_3_col(
            ...
            nhs_selectInput(
              ...
              choices = carehomes2::mod_geo_ch_flag_drug_df %>%
                dplyr::filter(GEOGRAPHY_PARENT == "Region", BNF_PARENT == "Chapter") %>%
        ...
        tabPanel(
          ...
          nhs_grid_3_col(
            ...
            nhs_selectInput(
              ...
              label = "BNF Type",
              choices = carehomes2::mod_geo_ch_flag_drug_df %>%
                dplyr::filter(GEOGRAPHY_PARENT == "ICS", BNF_PARENT == "Chapter") %>%
                dplyr::pull(BNF_CHILD) %>%
                unique() %>%
                sort(),
              full_width = TRUE
  ...
  moduleServer(id, function(input, output, session) {
    ...
    region_df <- reactive({
      ...
      if (startsWith(input$input_region_metric, "Total")) {
        df <- df %>%
          dplyr::mutate(dplyr::across(-dplyr::starts_with("GEOGRAPHY"), bespoke_round))
      }
        ...
        reactable::reactable(
          ...
          defaultColDef = reactable::colDef(
            ...
            cell = function(val, row, col_name) {
              if (col_name %in% c(names(geographies), ".selection")) {
                return(val)
              }

              accuracy <- ifelse(startsWith(metric, "Total") & (val < 10^3), 1, 0.01)
    ...
    output$ics_title <- renderUI({
      ...
      tags$div(
        style = "text-align: center;",
        tags$text(style = "font-weight: bold; font-size: 12pt;", selected_ics())
      )
    })
    ...
    output$lad_title <- renderUI({
      ...
      tags$div(
        style = "text-align: center;",
        tags$text(style = "font-weight: bold; font-size: 12pt;", selected_lad())
      )
    })
    ...
    create_download_data <- function(data) {
      tryCatch(return(
        carehomes2::bnf_level_prescribing_estimates_in_care_homes_df
      ), error = \(e) {
        data <- data %>%
          tidyr::pivot_wider(
          ...
          dplyr::mutate(dplyr::across(dplyr::starts_with("Total"), bespoke_round))
